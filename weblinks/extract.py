...
def clean_pages(pages: list[tuple[str, str]]) -> str:
    """Extract main-content text from each page and concatenate (URL-labeled).

    Pages are sorted by URL so the combined text — and therefore its hash — is
    stable regardless of crawl order.
    """
    parts: list[str] = []
...
def page_hash(clean_text: str) -> str:
    """Baseline fingerprint of a site, mixing in SCHEMA_VERSION so a schema/prompt
...
def extract_structured(clean_text: str) -> dict:
    """Call Gemini with the response schema; return the parsed dict.

    Returns {} if the model output can't be parsed as JSON.
    """
